...
| NewArray : Expr -> Expr (* an array consisting of contiguous memory cells *)
| Deref : Expr -> Expr (* read a single value from the memory *)
| Shift : Expr -> Expr -> Expr (* shift a label by a nonnegative integer *)
| Assign : Expr -> Expr -> Expr
| Free : Expr -> Expr (* free the given memory cell *)
| Seq : Expr -> Expr -> Expr
...
Fixpoint map_v (f : string -> string) (v : Value) : Value :=
  match v with
  | U_val => U_val
  | Var x => Var (f x)
  | Int i => Int i
...
with map_e (f : string -> string) (e : Expr) : Expr :=
  match e with
  | Val v => map_v f v
